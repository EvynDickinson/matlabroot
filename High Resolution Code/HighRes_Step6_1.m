...
if strcmp(questdlg('Load existing data structure?'),'Yes')
    ...
    switch questdlg('How do you want to load data?', '', ...
                'manually from excel', 'structure list', 'Cancel','structure list')
        case 'manually from excel'
...
% % override the saved path location variables: 
% baseFolder = []; 
% baseDir = [];
% groupDir = dir_path;
% figDir = createFolder([groupDir '/Figures/']);
% initial_var{end+1} = 'hold_exp'; % this may not be already included...
% fps = 30;
% initial_var{end+1} = 'fps';
% 
% clearvars('-except',initial_var{:})
...
sexlist = {'m', 'f'};
MT_pos = nan([max_len, length(fieldnames(body)), num.trials]);
[data.x_loc(M).pos, data.x_loc(F).pos, data.y_loc(M).pos, data.y_loc(F).pos]  = deal(MT_pos);
...
for i = 1:num.trials
    ...
    for sex = 1:2
        ...
        data.sleep(new_idx,sex,i) = fly(i).(sexlist{sex}).sleep(curr_idx);
    end
end

data.time = fly(1).time;
data.color(M,:) = fly(1).data(M).color; % male color
data.color(F,:) = fly(1).data(F).color; % female color
