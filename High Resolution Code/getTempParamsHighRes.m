...
switch protocol
    ...
    case 'HR_LTS_R_15-35'
        param.ntrans = 4;
        param.labelstr = 'click (1) the start of the ramp, (2) the trough, (3) the peak, and (4) the end of the ramp';
        % find the x-time value for each time period
        tRate = struct;
        tRate(1).name = 'start hold';
        tRate(1).color = Color('Gray');        
        ...
        tRate(4).name = 'cooling';
        tRate(4).color = Color('dodgerblue');
        tRate(5).name = 'end hold';
        tRate(5).color = Color('Gray');
        param.tRate = tRate;
