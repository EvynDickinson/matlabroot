...
for trial_idx = 1:length(trial_options)
    ...
    % Process the data: 
    save_status = batchProcessData(processed_path);
    if save_status
...
function save_status = batchProcessData(filePath)

    %% update variables to save in this structure
    load(filePath)
    initial_var{end+1} = 'max_gap';
    max_gap = 1;
    initial_var{end+1} = 'filePath';
...
%% save and update the data structure
    version_name = 'v_5.1.2 | max gap 1';

    try
        save(filePath, 'T', 'data','version_name', '-append', '-v7.3')
        save_status = true;
    catch ME
