...
x_offset = data(F).rawX(:,body.center);
y_offset = data(F).rawY(:,body.center);
mx = data(M).rawX-x_offset;
...
theta = data(M).mfbodyangle; % angle between the fly bodies (both neg and positive)
test = theta<90;
...
scatter(x(:,1),y(:,1),15,data(M).color,"filled","^")
...
BL = 2.5/pix2mm; % body length (in pixels)

% Body position locations
...
% Likely 
BLidx = abs(mX(:,body.center)) <= (5*BL) & abs(mY(:,body.center)) <= (5*BL);
roi1 = r.b & r.c & r.f & r.g & BLidx;
roi2 = r.a & r.c & r.f & r.h & BLidx;
roi3 = r.d & r.b & r.e & r.g & BLidx;
roi4 = r.a & r.d & r.e & r.h & BLidx;
...
loc = gray | gray2 | likely;
T.courtposition = loc;  % yes = green, no = red
