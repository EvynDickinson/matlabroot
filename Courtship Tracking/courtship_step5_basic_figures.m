...
xlimit = [time(m.chaseroi(i,1)),time(m.chaseroi(i,2))];
...
for i = 1:size(m.chaseroi,1)
    ...
    subplot(r,c,sb(5).idx)
        % Plot IFD timecourse
        plot(time,data(M).wingspread,'color', foreColor,'LineWidth', lw)
        % Axes labels and limits
        ylabel('male wings angle (\circ)')
        set(gca, 'xcolor', 'none')
        xlim(xlimit)
    % 5) Body positions overlaid on arena image 
    subplot(r, c, sb(2).idx)
        % Plot arena image
        imshow(img)
        % Plot fly centers over the course of the chase bout
...
for i = offset:n
    binedges = [T.frame(i):T.frame(binwidth + i)];
    sp_var(i) = std(m.speed(binedges));
end
...
% calc start 
% calc stop
% run std for speed in that region
% save that std
% move to next one
% plot over time
