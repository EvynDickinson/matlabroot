...
%resize the data:
    ...
    data(vid).x_loc_new = X; % save for later convience
    data(vid).y_loc_new = Y;
end
% number of tracked points:
overtracked(1) = sum(raw_flyCount)-sum(flyCount);
overtracked(2) = sum(flyCount)-sum(new_flyCount);
...
%% Find well count + distance from each well
...
overtrackedFlies = sum(raw_flyCount-flyCount);
