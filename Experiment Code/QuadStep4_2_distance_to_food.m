...
%% FIGURE: Flies on food analysis TODO: update this to the new 'grouping' based version of the data storage
clearvars('-except',initial_vars{:})
plot_err = true;
[foreColor,backColor] = formattingColors(blkbgd);
num_lim = [0,5];
...
fig = getfig('',true);
    ...
    subplot(r,c,sb(3).idx); hold on
        ...
        % % plot(x,y,'LineWidth',LW,'Color',foreColor)
        % yyaxis right
        % y = smooth(grouped(i).fliesonfood.counts(:,trial),sSpan,'moving');
        % plot(x,y,'LineWidth',LW,'Color','red')
    ...
    subplot(r,c,sb(4).idx); hold on
        ...
        % yyaxis right
        % % flies on food
        % x = grouped(i).fliesonfood.temps;
        % yUp = grouped(i).fliesonfood.increasing.count_raw(:,trial);
        % yDown = grouped(i).fliesonfood.decreasing.count_raw(:,trial);
        % plot(x,yUp,'color','r','linewidth',LW+1,'linestyle', '-')
        % plot(x,yDown,'color','r','linewidth',LW+1,'linestyle', '--')
...
subplot(r,c,sb(1).idx)
ylabel('\circC')
set(gca,"XColor",'none','ytick',[15,23])

% distance
subplot(r,c,sb(2).idx)
ylabel('flies in region (%)')
ylim([0,x_max])
set(gca,"XColor",'none','ytick',0:20:x_max)
legend({'quad', 'circle', 'ring','sleep'},'location', 'northwest', 'box', 'off','fontsize', 8,'textcolor', foreColor)
% speed
subplot(r,c,sb(3).idx)
ylabel('speed (mm/s)')
xlabel('time (min)')
...
for i = 1:num.exp 
    ...
    fig = getfig('',true);
        ...
        subplot(r,c,sb(3).idx); hold on
            ...
            % % plot(x,y,'LineWidth',LW,'Color',foreColor)
            % yyaxis right
            % y = mean(grouped(i).fliesonfood.counts,2,'omitnan');
            % y = smooth(y,sSpan,'moving');
        ...
        subplot(r,c,sb(4).idx); hold on
            ...
            k = kolor;
            x = grouped(i).fliesonfood.temps;
            yUp = grouped(i).fliesonfood.increasing.count_avg;
            yDown = grouped(i).fliesonfood.decreasing.count_avg;
            plot(x,yUp,'color','m','linewidth',LW+1,'linestyle', '-')
            plot(x,yDown,'color','m','linewidth',LW+1,'linestyle', '--') 
...
%% FIGURE: [LRR only] Max flies in outer ring during a temp ramp
clearvars('-except',initial_vars{:})
...
for i = 1:num.exp
    exp = (exp_sel(i));
    y = max(grouped(exp).ring.decreasing.raw(1:4,:));
    x = i*ones(1,num.trial(exp));
...
formatFig(fig,blkbgd); 
set(gca, 'xcolor', 'none')
ylabel({'Max flies in outer ring (%)'; '   '})
ylim(y_lims)
set(gca,'ytick', y_lims(1):20:y_lims(2),'FontSize', 20,'TickDir','in')
h_line(25, 'grey', '--',1.5)

save_figure(fig,[saveDir 'Max Ring Occupancy cooling'],fig_type);



%% FIGURE: comparison across temp bins between parental control and manipulation trials
...
plot_err = true; % plot SEM
plot_high_null = true; % plot the low or high null occupancy for empty trials
[~,~,foreColor] = formattingColors(blkbgd,1); % get foreground color for current background configuration

narrow_fig = true;
narrow_fig_size = [600 680];
...
for i = 1:c
    ...
    % presentation code updates: (to be removed later)
    set(gca, 'xtick', 15:10:35)
...
for i = 1:nComps
    ...
    g1 = comp_pairs(i,1);
    g2 = comp_pairs(i,2);
    kolor = grouped(comp_pairs(i,3)).color;

    % save stats comparison information for later processing / display
    stats_comp(i).comp = [g1, g2];
    stats_comp(i).names = {grouped(g1).name, grouped(g2).name};
    stats_comp(i).temp = statsD(g1).temps;
    ...
    for type = 1:2 % heating and cooling
        ...
        % find number of comparisons that were made: 
        numberTests = max([sum(~all(isnan(x),2)), sum(~all(isnan(y),2))]);
        % multiple comparison correction on p-value
        ogAlpha = 0.05; %baseline alpha level for significance
        adjAlpha = ogAlpha / numberTests; % adjusted alpha level for significance
        h = p < adjAlpha; % points that ARE significant = true

        % save the significance location and direction for later use: 
        stats_comp(i).(type_name) = h;

        % make a list of locations that are significant to be later scale to
        % the appropriate y height to plot above the tuning curve
        plotYloc = nan(size(h));
        plotYloc(h) = 1; % 
        ...
        plotX = statsD(g1).temps;
        plotY = y_point*(plotYloc);
...
%% WORKING HERE: FIGURE: comparison across VARIABLE SIZED temp bins between parental control and manipulation trials
% this section will make a figure that shows the temp - XXX tuning curve,
% separated by heating and cooling. Additionally, this will plot simple
% t-test comparisons between the experimental group compared to each of the
% parental control groups, so we can see the areas in which the
...
switch questdlg('Plot food occupation or ring occupation?','', 'Food', 'Ring', 'Speed', 'Food')
    case 'Food'
        ylab = 'flies in food quadrant (%)';
        fig_title = ['Food occupancy 15-23 scatter'];
        data_name = 'occ';
        data_scaling = 100;
        null_line = 14.4;
    case 'Ring'
        ylab = 'flies in outer ring (%)';
        fig_title = ['Ring occupancy 15-23 scatter'];
        data_name = 'ring';
        data_scaling = 1;
        null_line = 25;
    case 'Speed'
        ylab = 'fly movement speed (mm/s)';
        fig_title = ['Fly speed 15-23 scatter'];
        data_name = 'speed';
        data_scaling = 1;
        % null_line = 50;
    case ''
...
% expList = [5 5 1 1 4 4 2 2 7 7 3 3 6 6]; % experiment group order
% tempList = [17 25]; % temperatures for each data set
% tempList = repmat(tempList, [1, length(expList)/2]);

% expList = [1 1 3 3 2 2]; % experiment group order
...
cList = repmat(cList, [1, length(expList)/2]);
...
% ttest comparisons within group
g1 = testData(:,1:2:end);
g2 = testData(:,2:2:end);

p = [];
for exp = 1:num.exp
    [y1, y2] = rmnan(g1(:,exp), g2(:,exp), 1, true);
    [~, p(exp)] = ttest(y1,y2);
end
% TODO: bonferonni correction of the p-value
alpha = 0.05;
corrected_alpha = alpha/num.exp;
h = p<=corrected_alpha;
disp(expNames(h)')
...
for i = 1:num.exp
    kolor = grouped(i).color;
    %increasing
    x = grouped(i).fullquad.food.temps;
    y = grouped(i).fullquad.food.increasing.avg;
    y_err = grouped(i).fullquad.food.increasing.std;
    loc = isnan(y) | isnan(y_err);% remove nans
    y(loc) = []; x(loc) = []; y_err(loc) = [];
    plot_error_fills(plot_err, x, y, y_err, kolor,  fig_type, 0.2);
    plot(x,y,'LineWidth',LW+0.5,'Color',kolor,'linestyle','-')
    %decreasing
    x = grouped(i).fullquad.food.temps;
    y = grouped(i).fullquad.food.decreasing.avg;
    y_err = grouped(i).fullquad.food.decreasing.std;
    loc = isnan(y) | isnan(y_err);% remove nans
    y(loc) = []; x(loc) = []; y_err(loc) = [];
...
hold on
for i = 1:num.exp
    x = repmat(grouped(i).fullquad.food.temps,[num.trial(i),1]);
    y = grouped(i).fullquad.food.decreasing.raw-grouped(i).fullquad.food.increasing.raw;
    % [x, y] = rmnan(y, x, 1, true);
    kolor = grouped(i).color;
%     plot(x,y,'color',kolor,'LineWidth',LW);
    plot(mean(x,2),mean(y,2),'color',kolor,'LineWidth',2)
end
h_line(0,foreColor,':',1)
xlabel('temp (\circC)')
ylabel('food quadrant occupancy difference (%)')
