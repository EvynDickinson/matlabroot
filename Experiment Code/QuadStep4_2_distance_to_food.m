...
%% FIGURE: plot food occupancy vs ring occupancy but color coded by temperature
clearvars('-except',initial_vars{:})
sz = 50;
...
for exp = 1:num.exp
    temp = grouped(exp).occ.temps;
    cmap = [Color('blue', 'grey', sum(temp<=25)); Color('grey','red', sum(temp>25))];

    x = grouped(exp).occ.increasing.avg.*100;
    y = grouped(exp).ring.increasing.avg;
    plot(x,y,'color', grouped(exp).color, 'linewidth', 1)
    scatter(x,y,sz, cmap,'filled')
    
    x = grouped(exp).occ.decreasing.avg.*100;
    y = grouped(exp).ring.decreasing.avg;
    plot(x,y,'color', grouped(exp).color, 'linewidth', 1,'linestyle', ':')
    scatter(x,y,sz, cmap)
end
xlim([0,100])
ylim([0, 100])
