...
%% FIGURE (TEMP HOLDS ONLY):  [ OLD ] 2D spatial occupancy in arena histogram
...
for i = num.exp
    ...
    r = data(i).data(1).data.r;  % pixel radius of the arena (for trial 1)...since only plate-aligned trials will be used eventually
    Cx = mean(grouped(i).position.well_pos.x(5,:)); %center X
    Cy = mean(grouped(i).position.well_pos.y(5,:)); %center Y
    x_edge = linspace(Cx-r,Cx+r,n);
    y_edge = linspace(Cy-r,Cy+r,n);
...
[foreColor] = formattingColors(blkbgd);
acceptable_plate_conds = [1 2 3]; % which plate conditions are allowable 
% Find the occupancy for each bin:
n = 26; % number of spatial bins
autoLim = false;
% axis_limits = [0, 1.5];
...
for i = 1:length(temp_list) % could also do this as auto find of the avg temp for the trial...
    ...
    for type = 1:nTypes % e.g. food quad only, or low and high occ quad orientations
        ...
        for rr = 1:nRates % cycle through the temp ratesssss
            ...
            for trial = 1:num.trial(exp)
                con_type = data(exp).con_type(trial);
                if any(con_type==acceptable_plate_conds) %check if it matches desired plate 
...
fig = getfig('',false,[fig_W, (340*r)]); 
for i = 1:nTemps
    % .... subtract one from the other but add in a dynamic switch for low or high occupancy comparisions
...
z0 = -4; z1 = 0; z2 = 4; % color axis limits...
...
%% LOAD AND COMPARE STATIC TRIALS ACROSS TEMPERATURES TO 25C
% THIS IS MANUALLY CONTROLLED AND CANNOT BE AUTO-RUN
clearvars('-except',initial_vars{:})
[foreColor] = formattingColors(blkbgd);
...
fig_type = '-pdf';
blkbgd = false;
color_lims = 4; % max color percent
...
FT = false; % Fictive time
reference_temp = 25; % which temperature is the reference temp 
food_type = 1; % food well position based data
empty_type = 3; % high occupancy based data

% find the exp number for the reference temperature
empty_ref = find([empty(:).plotData.real_temp]==reference_temp);
food_ref = find([food(:).plotData.real_temp]==reference_temp);
...
for type = 1:2 % food and then empty
    maxDiff = zeros(1,nTemps);
    fig = getfig('',false,[fig_W, (340*r)]); 
    for i = 1:nTemps
        switch type
            case 1 % food
                n = food.plotData(i).type(food_type).data - food.plotData(food_ref).type(food_type).data;
                circSize = food.plotData(i).type(food_type).arenaSize(2:3,1)';
                circ_r = food.plotData(i).type(food_type).arenaSize(1,1);
                fig_title = ['Food temp comparison to ' num2str(reference_temp) 'C'];
            case 2 % empty
                n = empty.plotData(i).type(empty_type).data - empty.plotData(empty_ref).type(empty_type).data;
                circSize = empty.plotData(i).type(empty_type).arenaSize(2:3,1)';
                circ_r = empty.plotData(i).type(empty_type).arenaSize(1,1);
                fig_title = ['No food temp comparison to ' num2str(reference_temp) 'C'];
        end
        maxDiff(i) = max(max(n));
        subplot(r,c,i)
            hold on
    ...
    % Create custom colormap for positive and negative change between static and dynamic trials
    n = 256;  % number of colors
    z0 = -color_lims; z1 = 0; z2 = color_lims; % color axis limits...
    % Number of colors below and above zero
    n_neg = round(n * abs(z0) / (z2 - z0));
    n_pos = n - n_neg;
    switch blkbgd
    ...
    for i = 1: nTemps 
        ...
        clim([z0, z2])
        colormap(cmap)
        title([num2str(food.plotData(i).real_temp) '\circC | max: ' num2str(maxDiff(i))],'color', foreColor)
    end
    % save the figure
    save_figure(fig,[start_folder fig_title ' 2D spatial distribution difference'], fig_type);
end

% TODO:  PROBABLY NEED TO DO SOME KIND OF BOOTSTRAPPING HERE TO SHOW THAT THERE
% ISN'T REALLY A STRONG TEMPERATURE DIFFERENCE??? OR JUST RELY ON THE
% AREA-SPECIFIC MEASURES THAT COME LATER IN THE FIGURE
