...
for i = 1:num.exp
    T = data(i).T;
    for trial = 1:num.trial(i)
        sleep_file = [baseFolder T.Date{trial} '\Arena ' T.Arena{trial} '\' T.ExperimentID{trial} ' sleeping.mat'];  
        if ~exist(sleep_file,"file")
            sleepData = struct;
            %preallocate for speed and space
            trial_length = length(data(i).data(trial).occupancy.time);
            [N,frameCount,sleepingCount,sleepLoc] = deal(nan(nbins,nbins,trial_length));
...
%% NEW SLEEP METHODS
 ...
 pixel_movement = 20; %allowed 'movement' in a pixel (based on a 50x50 grid space across the image or about a 1/2 fly body length)
 ...
 for i = 1:num.exp
     for trial = 1:num.trial(i)
         ...
         if ~exist(sleep_file,"file")
            ...
            % image of fly position with movement buffer zone: 
...
% Find the coordinates of the sleeping flies bins from discretize
test_nums = 1:49;

bin_size = 7;
y_row = ceil(test_nums/bin_size);
x_row = rem(test_nums-1,bin_size)+1;
