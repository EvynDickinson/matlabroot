...
names = {'hikone','harwich','crimea','bogota_2','bogota_3','Zambia','lousiana','smarkand', 'portugal','athens','athens','urbana'};
coords = [35.27,136.26;...   hikone
          ...
          4.71, -74.07;...  bogota
          -16.92, 28;...    Zambia
          30.98, -91.96;... lousiana (new orleans)
          39.63, 66.97;...  samarkand
...
%% 
rand_sel = shuffle_data(linspace(-0.15,0.15,length(stocks)));
...
%% 
rand_sel = shuffle_data(linspace(-0.2,0.2,length(stocks)));

fig = figure; set(fig, 'pos',[-843 481 775 292]); hold on
% x = shuffle_data(linspace(1,2,length(stocks)));
for i = 1:length(stocks)
    if i<=curr_stocks
        x = 1;
    ...
    scatter(stocks(i).coor(2), x+rand_sel(i), 50, stocks(i).color, 'filled')
end
xlim([0.5,2.5])
...
save_figure(fig,[rootdir 'Figures/Genotype Longitude'],'-png');
