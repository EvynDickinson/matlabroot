...
% Fit the data:
for ii = 1:3
    [filteredData,coefficients,xFit,yFit,sortedData,yfit,] = deal([]); 
    switch ii
        case 1 % plant food
            inputData = plant;
    ...
    filteredData(:,1) = smoothdata(sortedData(:,1),'movmean',sSpan);
    filteredData(:,2) = smoothdata(sortedData(:,2),'movmean',sSpan);
    switch FitType
        case 'Smoothed data'
    ...
    plotData(ii).smoothed = filteredData;
...
subplot(nrows, ncols, sb(2).idx) 
    hold on
    scatter(smoothdata(sortedData(:,1),'movmean',sSpan),...
            smoothdata(sortedData(:,2),'movmean',sSpan),10,kolor,'filled')
    xlim(xlimits)
    xlabel('Temperature (\circC)')
    ylabel('Movement (au)')
...
%     plotData(gg,ii).rawData = sortedData;
    plotData(gg,ii).smoothed = filteredData;
...
for arena = 1:4
  for well = 1:4
    ...
    R = R(2,1);
    % save data for plotting:
    plotData(arena,well).smoothed = filteredData;
...
for ii = 1:6 %for each well type
    ...
    filteredData(:,2) = smoothdata(sortedData(:,2),'movmean',sSpan);
    % find the line of best fit:
    coefficients = polyfit(filteredData(:,1), filteredData(:,2),1);
    xFit = linspace(filteredData(1,1), filteredData(end,1), numFitPoints);
    yFit = polyval(coefficients, xFit);
    yfit = polyval(coefficients, filteredData(:,1));
    yResid = filteredData(:,2)-yfit;
    SSresid = sum(yResid.^2);
    SStotal = (length(filteredData(:,2))-1) * var(filteredData(:,2));
    rsq = 1 - SSresid/SStotal; % get the R-square value
    % coefficient of correlation
    R = corrcoef(filteredData(:,1),filteredData(:,2));
    R = R(2,1);
    % save data for plotting:
    plotData(ii).smoothed = filteredData;
    plotData(ii).bestfit = [filteredData(:,1),yfit];
    plotData(ii).color = kolor;
    plotData(ii).rsq = round(rsq,2);
    plotData(ii).R = round(R,2);
end
...
subplot(nrows, ncols, subs(2).idx)
    sSpan = 240;
    hold on
    Y = [smoothdata(mean(A,2),'movmean',sSpan), smoothdata(mean(B,2),'movmean',sSpan),...
         smoothdata(mean(C,2),'movmean',sSpan), smoothdata(mean(D,2),'movmean',sSpan)];
    h = area(TAT,Y);
    for well = 1:4
