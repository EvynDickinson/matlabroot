...
if ~exist(well_loc_file,'file') || ~exist(well_coor_file,'file')
...
% (in case there are videos that did not track well and need to be
% adjusted)
d = [];
fig = getfig('',1,[800 900]);
for vid = 1:nvids
    loc = T.vidNums==vid;
    a = flyCount(loc,:);
    b = mean(a,1,'omitnan');
...
save_figure(fig, [analysisDir expName ' Fly Count by video'],'-png',false,true,'-r80');
% print out how many frames were completely untracked: 
fig = getfig('',1,[800 900]);
for i = 1:4
    subplot(2,2,i)
    plot(d(:,1), d(:,i+1), 'color', arenaData(i).color,'linewidth', 2)
    xlabel('video number')
    ylabel('total dropped frames')
    title(['Arena ' Alphabet(i)],'color', 'w')
end
formatFig(fig, true,[2 2]);
save_figure(fig, [analysisDir expName ' Dropped frames by video'],'-png',false,true,'-r80');

% option to display videos that could be retracked for skipped frames
...
if strcmp(questdlg('Is the tracking okay to continue?'),'No')
    disp('Canceled further processing --> retrack and try again')
    return
end


%% Display total numbers of flies that have been tracked across the trials
...
%% Select persisent extra tracks for deletion if needed
...
switch response
    case 'Yes'
        ...
        subplot(row,col,sb(9).idx)
            set(gca,'FontSize',10)
            legend(labels,'TextColor', 'white', 'location', 'northwest', 'box', 'off','fontsize', 7)
        save_figure(fig,[figDir expName ' radius selection figure'],'-png')
        
        % Display radius size with smallest cumulative sum
        [~,iidx] = find(cumulative_sum == min(cumulative_sum));
        PR = full_list(iidx); % radius around point
        disp(['Radius size: ', num2str(PR)]);
        initial_vars = [initial_vars; 'PR'];
