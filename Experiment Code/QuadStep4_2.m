...
% TODO 5.13 update this to account for different plates
...
for exp = 1:num.exp
    for trial = 1:num.trial(exp)
        ...
        for i = 1:length(food_fields)
            for q = 1:4
                sROImask(exp).(food_fields{i}).(quadOrder{q})(trial).m = [];
            end
        end
    end
end
...
for exp = 1:num.exp
    ...
    for rr = 1:length(regionList)
        for q = 1:4 % each of the quadrants
            region(rr).(quadOrder{q}).allflies.all = [];
            region(rr).(quadOrder{q}).allflies.all_info = {['percent of sleeping flies in the region relative ...' ...
                                        'to all the flies (not just sleeping flies) in the whole arena']};
            region(rr).(quadOrder{q}).sleepflies.all = [];
            region(rr).(quadOrder{q}).sleepflies.all_info = {'in region percent of the total flies that are sleeping'};
            region(rr).(quadOrder{q}).withinROI.all = [];
            region(rr).(quadOrder{q}).withinROI.all_info = {'in region sleep percent of the total flies in that region'};
            if strcmp(regionList{rr},'innerquad') | strcmp(regionList{rr},'quadring')
                region(rr).(quadOrder{q}).allflies.partial = [];
                region(rr).(quadOrder{q}).allflies.partial_info = {'percent relative to only the total flies in the four quadrants of this space'};
                region(rr).(quadOrder{q}).sleepflies.partial = [];
                region(rr).(quadOrder{q}).sleepflies.partial_info = {'percent relative to only the sleeping flies in the four quadrants of this space'};
            end
        end
    end
    ...
    for trial = 1:num.trial(exp)
        ...
        for rr = 1:length(regionList)
            for q = 1:4 % each of the quadrants
                temp = sROImask(exp).(regionList{rr}).(quadOrder{q})(trial).m;
                for g = 1:3 % for each percentage type (e.g. all flies, within sleeping flies or within the region
                    switch g
                        ...
                        case 3 % within the flies of that region
                            nArea = sum(ROImask(exp).(regionList{rr}).(quadOrder{q})(trial).m,2);
                            n = getPercentFlies(temp,nArea);
                    end
                    
                    region(rr).(quadOrder{q}).(groupType{g}).all = autoCat(region(rr).(quadOrder{q}).(groupType{g}).all,n,false);
                end
                if strcmp(regionList{rr},'innerquad')
                    n = getPercentFlies(temp,nInner);
                    region(rr).(quadOrder{q}).allflies.partial = autoCat(region(rr).(quadOrder{q}).allflies.partial,n,false);
                    n = getPercentFlies(temp,snInner);
                    region(rr).(quadOrder{q}).sleepflies.partial = autoCat(region(rr).(quadOrder{q}).sleepflies.partial,n,false);
                elseif strcmp(regionList{rr},'quadring')
                    n = getPercentFlies(temp,nRing);
                    region(rr).(quadOrder{q}).allflies.partial = autoCat(region(rr).(quadOrder{q}).allflies.partial,n,false);
    ...
    for rr = 1:length(regionList)
        for q = 1:4 % each of the quadrants
            for g = 1:3
                region(rr).(quadOrder{q}).(groupType{g}).avg = mean(region(rr).(quadOrder{q}).(groupType{g}).all,2,'omitnan');
                region(rr).(quadOrder{q}).(groupType{g}).std = std(region(rr).(quadOrder{q}).(groupType{g}).all,0,2,'omitnan');
            end
            if strcmp(regionList{rr},'innerquad') | strcmp(regionList{rr},'quadring')
                region(rr).(quadOrder{q}).allflies.partial_avg = mean(region(rr).(quadOrder{q}).allflies.partial,2,'omitnan');
                region(rr).(quadOrder{q}).allflies.parital_std = std(region(rr).(quadOrder{q}).allflies.partial,0,2,'omitnan');
                region(rr).(quadOrder{q}).sleepflies.partial_avg = mean(region(rr).(quadOrder{q}).sleepflies.partial,2,'omitnan');
                region(rr).(quadOrder{q}).sleepflies.parital_std = std(region(rr).(quadOrder{q}).sleepflies.partial,0,2,'omitnan');
            end
        end
    end   
      ...
      for g = 1:3
        ...
        for p = 1:nP % for the number of full or partial percentages to compare
            for q = 1:nQ % each of the quadrants
                ...
                % pull data for the right quadrant (if there are quadrants)
                if nQ>1
                    baseY = grouped(exp).sleep.(all_regions{rr}).(quadOrder{q}).(groupType{g});
                else 
                ...
                switch all_regions{rr}
                    ...
                    case {'fullquad','innerquad','quadring'}
                        if p == 1
                            grouped(exp).sleep.(all_regions{rr}).(quadOrder{q}).(groupType{g}).increasing.raw = raw_h;
                            grouped(exp).sleep.(all_regions{rr}).(quadOrder{q}).(groupType{g}).increasing.avg = mean(raw_h, 2, 'omitnan');
                            grouped(exp).sleep.(all_regions{rr}).(quadOrder{q}).(groupType{g}).increasing.std = std(raw_h, 0, 2, 'omitnan');
                            grouped(exp).sleep.(all_regions{rr}).(quadOrder{q}).(groupType{g}).decreasing.raw = raw_c;
                            grouped(exp).sleep.(all_regions{rr}).(quadOrder{q}).(groupType{g}).decreasing.avg = mean(raw_c, 2, 'omitnan');
                            grouped(exp).sleep.(all_regions{rr}).(quadOrder{q}).(groupType{g}).decreasing.std = std(raw_c, 0, 2, 'omitnan');
                            grouped(exp).sleep.(all_regions{rr}).(quadOrder{q}).(groupType{g}).temps = temps;
                        else
                            grouped(exp).sleep.(all_regions{rr}).(quadOrder{q}).(groupType{g}).increasing.p_raw = raw_h;
                            grouped(exp).sleep.(all_regions{rr}).(quadOrder{q}).(groupType{g}).increasing.p_avg = mean(raw_h, 2, 'omitnan');
                            grouped(exp).sleep.(all_regions{rr}).(quadOrder{q}).(groupType{g}).increasing.p_std = std(raw_h, 0, 2, 'omitnan');
                            grouped(exp).sleep.(all_regions{rr}).(quadOrder{q}).(groupType{g}).decreasing.p_raw = raw_c;
                            grouped(exp).sleep.(all_regions{rr}).(quadOrder{q}).(groupType{g}).decreasing.p_avg = mean(raw_c, 2, 'omitnan');
                            grouped(exp).sleep.(all_regions{rr}).(quadOrder{q}).(groupType{g}).decreasing.p_std = std(raw_c, 0, 2, 'omitnan');
                        end
                    case {'circle10','circle7','circle5'}
                        grouped(exp).sleep.(all_regions{rr}).(quadOrder{q}).(groupType{g}).increasing.raw = raw_h;
                        grouped(exp).sleep.(all_regions{rr}).(quadOrder{q}).(groupType{g}).increasing.avg = mean(raw_h, 2, 'omitnan');
