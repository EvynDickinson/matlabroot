...
switch run_ans
    case 'Yes'
        ...
        for trial = 1:ntrials
            ...
            dirc = [baseFolder, trialDate, '/Arena ' trialArena '/analysis/' trialExpID trialArena  ' timecourse data v2.mat'];

            % TODO 5/26/25: update this version to allow for speed and
            % sleep data loading (from the excel derived structures) 
...
for trial = 1:ntrials
    ...
    if isempty(data(trial).dist2wells)
        try
            ...
            for well = 1:4
                data(trial).dist2wells(:,well) = data(trial).occupancy.dist2wells(well).N(:,1)./data(trial).pix2mm;
            end
        end
    elseif isfield(data(trial).dist2wells, 'N')
        ...
        for well = 1:4
            data(trial).dist2wells(:,well) = temp(well).N(:,1)./data(trial).pix2mm;
        end
    end
end
...
% Generate fictive temperature for 'hold' experiments
initial_vars{end+1} = 'temp_protocol';
...
if hold_exp
     for trial = 1:ntrials
        ...
        lower_temp = tP.threshLow+0.5;
        % set full temp length
        fict_temp =  upper_temp*ones(size(data(trial).occupancy.temp));

        %down ramps
        for i = 1:tP.nDown
            fill_temp = linspace(upper_temp,lower_temp,tP.down(i,2)-tP.down(i,1)+1);
            fict_temp(tP.down(i,1):tP.down(i,2)) = fill_temp;
        end
        %up ramps
        for i = 1:tP.nUp
...
% TODO: update this to work for 'mock' temperature protocol to check time course not
...
%     end
    ...
    dT = diff(smooth(temp,180));  %1 minute smoothing kernal (for 3fps data)
    dt = diff(time); 
    plotData(:,2) = dT./dt; 
    ...
    if strcmp(T.TempProtocol{trial},'linear_ramp_with_recovery_23-15') || ...
       ...
       strcmp(T.TempProtocol{trial},'linear_ramp_M_25-17')|| ...
...
%        figure; hold on; plot(rateData); plot(plotData(:,4))     
    ...
    TR.rates = T_rates;
    TR.data = plotData;
    TR.dataNames = {'temp', 'change in time', 'distance from food', 'rate of temp change', 'rate bin index'};
