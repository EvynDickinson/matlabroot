...
Rin3=theta(5);  % input resistance, MOhm
tau3=1e-3*theta(6);  % one time constant, ms->s
R1=theta(7);  % MOhm
C1=1e-6*theta(8);  % pF->uF
R2=theta(9);  % MOhm
C2=1e-6*theta(10);  % pF->uF
Cx=1e-6*theta(11);  % pF->uF

s=j*2*pi*f;  % rad/s
Z_m=Rin1./(1+tau1*s)+Rin2./(1+tau2*s)+Rin3./(1+tau3*s);  % MOhm
Y_m=1./Z_m;  % uS
