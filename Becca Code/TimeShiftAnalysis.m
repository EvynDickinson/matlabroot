...
%% FIGURE & STATS: Starting distance to food before cooling | heating across start times
...
%% STATS: ANOVA of distance from food, ramp, and Zeitgeber time
...
for trial = 1:ntrials
    ...
    for r = 1:3
        % Identify frames in the window at the start of each ramp
        rstart = tempPoints.hold(r,2)-nframes:tempPoints.hold(r,2);
        % rframe = tempPoints.down(r,1);
        % Calulate average distance to food at each ramp start
        rstartavg = mean(y(rstart));
        % Calculate Zeitgeber time (in hrs) of each ramp start
        zeit = expstarttime_hr(trial) - 8 + toffset(r); 
        stats = [stats; rstartavg, r, zeit];
        % stats = [stats; rstartavg, rframe, zeit];
        % R1 = [R1, r == 1];
        % R2 = [R2, r == 2];
        % R3 = [R3, r == 3];
    end
...
any(isnan(y))   % Check for NaNs in the dependent variable
any(isnan(g1))  % Check for NaNs in g1
any(isnan(g2))  % Check for NaNs in g2

g1 = stats(:, 2);  % The second column (group variable 1)
g2 = stats(:, 3);  % The third column (group variable 2)

% Check the unique values of g1 and g2
disp(unique(g1));  % Should have more than one unique value
disp(unique(g2));  % Should have more than one unique value

g1 = categorical(stats(:, 2));  % Convert group 1 to categorical
g2 = categorical(stats(:, 3));  % Convert group 2 to categorical



% ANOVA
% factors = {stats(:,2),stats(:,3)};
% aov = anova(factors,stats(:,1),FactorNames = ["Ramp" "Zeitgeber time"]);
% % aov = anova(stats(:,2),stats(:,1)); %,FactorNames = ["Ramp" "Zeitgeber time"]);
% disp(aov)
% 
% % two way with random effects
% tbl = table(stats(:,2),stats(:,3), VariableNames = ["Ramp" "Zeitgeber time"]);
% aov = anova(tbl,stats(:,1),CategoricalFactors=2,RandomFactors=2);
% disp(aov)
...
y = stats(:,1);
g1 = categorical(stats(:,2));  % Make sure these are categorical
g2 = categorical(stats(:,3));  % Make sure these are categorical
p = anovan(y, {g1, g2}, 'model', 'interaction', 'varnames', {'g1', 'g2'});
