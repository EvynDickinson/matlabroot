...
waserr = 0;
try
    h=findall(fig,'Type','uimenu','Accelerator',letter);   % Disabling ^C for edit menu so the only ^C is for
    ...
    set(h,'Accelerator',letter);                           % Set back the accelerator for edit menu.
catch %#ok<CTCH>
    waserr = 1;
end
drawnow;
if(waserr == 1)
    set(h,'Accelerator',letter);                          % Set back the accelerator if it errored out.
    error(message('MATLAB:ginput:Interrupted'));
end
...
for k = 1:4
    ...
    crossHair(k) = uicontrol(fig, 'Style', 'text', 'Visible', 'off', 'Units', 'pixels',...
        'BackgroundColor', ColorInput, 'HandleVisibility', 'off', 'HitTest', 'off'); %#ok<AGROW>
