function data = AcquireTrace(expnumber,numtrials,trialduration,stimulustime,stimulusduration,mode,VGain,IGain)
%expnumber = 1;
...
if ~isdir(['E:\Evyn\data\' todaysDate])
    mkdir(['E:\Evyn\data\' todaysDate]);
end
basepath = ['E:\Evyn\data\' todaysDate];
...
if isempty(D)
    ...
    load(['E:\Evyn\Data\' todaysDate,'\WCwaveform_',todaysDate,'_E',num2str(expnumber),'.mat']','data');
...
for k = 1:numtrials
    ...
    data(n).samprateout = 10E3;                           % output sample rate becomes input rate as well when both input and output present
    ...
    data(n).cameraTriggerVector = [zeros(1,data(n).samprateout*1) 5*ones(1,data(n).samprateout*1) zeros(1,data(n).samprateout*(trialduration-2))]';
    ...
    s.addAnalogOutputChannel('Dev1', [0:1 3] , 'Voltage');
    s.Rate = data(n).samprateout;
    
    s.queueOutputData([data(n).commandVector data(n).stimulusVector data(n).cameraTriggerVector]);
    x = s.startForeground();
