...
% [param.A_vid, param.A_Basler_src] = initiate_camera(param.CamA);
% [param.B_vid, param.B_Basler_src] = initiate_camera(param.CamB);
% [param.C_vid, param.C_Basler_src] = initiate_camera(param.CamC);
% [param.D_vid, param.A_Basler_src] = initiate_camera(param.CamA);
% [param.E_vid, param.B_Basler_src] = initiate_camera(param.CamB);
% [param.F_vid, param.C_Basler_src] = initiate_camera(param.CamC);
...
pause(param.start_pause); % Pause before starting stimulus procedure 
...
% ROTATE THROUGH ALL CONDITIONS:
  for jj = 1:length(conds) 
    ...
    for icam = 1:param.num_cams
        diskLogger = VideoWriter([temp.vid_strt num2str(ii) 'C' num2str(CC) ' Cam-' Alphabet(icam) ' ' temp.vid_end],...
            'Grayscale AVI');
...
%     stop(param.A_vid); stop(param.B_vid); stop(param.C_vid);stop(param.D_vid); stop(param.E_vid); stop(param.F_vid);
    ...
    param.writetodiskpause = 6; % param.target_loop_time-actual_loop_time;
